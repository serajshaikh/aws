...
exports.handler = async event => {
    ...
    const user = await Dynamo.deleteItem(ID, tableName).catch(err => {
        console.log('error to Deleting table', err);
        return null;
    });

    if (!user) {
        return Responses._Error400({ message: 'Failed to Delete user by ID'});
    }
